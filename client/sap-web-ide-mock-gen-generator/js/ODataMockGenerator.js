...
define([], function() {
    ...
    //copied and adjusted version from brackets-ui5 hence ES6 syntax, which is not 
    //preferred by SAP Web IDE extension development standard - other plugin 
    //files use ES5.
    return class ODataMockGenerator {
        constructor(metadataFileContent, mockDataConfigFileContent) {
            this._oMetadata = jQuery.parseXML(metadataFileContent);
            
            if (mockDataConfigFileContent) {
            	this._mockDataConfig = JSON.parse(mockDataConfigFileContent);
            }
            
            this._predefinedValuesConfig = this._mockDataConfig && this._mockDataConfig.predefined || {};
        	this._skipMockGeneration = this._mockDataConfig && this._mockDataConfig.skipMockGeneration || [];
        	this._rootUri = this._mockDataConfig && this._mockDataConfig.rootUri || "";
        	this._numberOfEntities = this._mockDataConfig && this._mockDataConfig.numberOfEntities || 30;
        ...
        createMockData() {
            ...
            let newFile, content, filename, result = [];
